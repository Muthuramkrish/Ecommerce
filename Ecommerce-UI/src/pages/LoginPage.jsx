...
const LoginPage = ({ onLoginSuccess }) => {
  ...
  const handleSignUp = async (e) => {
    ...
    if (!areAllPasswordRulesOk(password)) {
      setSignUpMsg({
        msg: "Please satisfy all password requirements.",
        error: true,
      });
      setShowSignUpChecklist(true);
      return;
    ...
    try {
      ...
      setSignUpMsg({
        msg: res.message || "Registration successful!",
        error: false,
      });
    ...
    <div className="fixed inset-0 flex items-center justify-center bg-gradient-to-br from-slate-100 via-blue-50 to-slate-100 p-2 sm:p-4 lg:p-6 overflow-hidden">
      <div className="hidden md:flex items-center justify-center rounded-3xl p-4">
        <img
          ...
          className="w-[75%] max-w-[350px] h-auto object-contain rounded-[2rem] shadow-2xl transition-transform duration-500 hover:scale-[1.02]"
        />
      </div>

      <div className="relative w-full max-w-[850px] h-full max-h-[550px] sm:max-h-[600px] lg:max-h-[550px] bg-white rounded-5xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] overflow-hidden flex items-center justify-center">
        {/* Back Button */}
        <button
          type="button"
          onClick={() => window.history.back()}
          className="absolute top-12 left-12 sm:top-12 sm:left-12 z-40 w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-full bg-white/90 hover:bg-white border border-slate-200 text-slate-700 hover:text-slate-900 transition-all shadow-md hover:shadow-lg"
          aria-label="Go back"
        >
        ...
        <div className="md:hidden absolute inset-0">
          <div
            className={`absolute inset-0 bg-gradient-to-br from-slate-50 to-blue-50 transition-transform duration-700 ease-in-out ${
        ...
        {/* Sign In Form */}
        <div
          className={`absolute top-0 left-0 w-full md:w-1/2 h-full flex items-center justify-center p-4 sm:p-6 lg:p-8 transition-all duration-700 ease-in-out z-10 ${
            active
              ? "opacity-0 pointer-events-none -translate-x-full md:translate-x-full"
              : "translate-x-0 opacity-100 pointer-events-auto"
          }`}
        >
          <div className="w-full max-w-[280px] overflow-y-auto max-h-full py-4 custom-scrollbar flex flex-col items-center">
            <div className="w-full" onSubmit={handleSignIn}>
              <h4 className="text-xl sm:text-xl lg:text-xl font-bold text-slate-800 text-center mb-3 sm:mb-4 lg:mb-6">
                Sign In
              </h4>

              <div className="flex justify-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                {[Facebook, Instagram, Mail, Github].map((Icon, i) => (
                  <button
                    key={i}
                    type="button"
                    className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center rounded-lg border-2 border-slate-300 hover:border-blue-600 hover:text-blue-600 transition-colors"
                  >
                    <Icon size={14} className="sm:w-3 sm:h-3 lg:w-4 lg:h-4" />
                  </button>
                ))}
              </div>

              <p className="text-xs sm:text-sm text-slate-500 text-center mb-3 sm:mb-4 lg:mb-6">
                or use email password
              </p>

              <div className="space-y-2 sm:space-y-2.5 lg:space-y-3 w-full">
                <input
                  type="email"
                  name="email"
                  placeholder="Email"
                ...
                <a
                  href="#"
                  className="block text-xs sm:text-sm text-blue-600 hover:text-blue-700 text-right transition-colors"
                >
                  Forgot password?
                </a>
                ...
                {signInMsg.msg && (
                  <div
                    className={`p-2 rounded-lg text-xs sm:text-sm text-center ${
                      signInMsg.error
        ...
        >
          <div className="w-full max-w-[280px] overflow-y-auto max-h-full py-4 custom-scrollbar flex flex-col items-center">
            <div className="w-full" onSubmit={handleSignUp}>
              <h4 className="text-xl sm:text-xl lg:text-xl font-bold text-slate-800 text-center mb-3 sm:mb-4 lg:mb-6">
                Create Account
              </h4>
              ...
              <div className="space-y-2 sm:space-y-2.5 lg:space-y-3 w-full">
                ...
                <div className="relative w-full">
                  <input
                    ...
                    onChange={handleSignUpChange}
                    onFocus={() => {
                      setSignUpPwdFocused(true);
                      setShowSignUpChecklist(true);
                    }}
                    onBlur={() => {
                      setSignUpPwdFocused(false);
                      setTimeout(() => setShowSignUpChecklist(false), 150);
                    }}
                    required
                    ref={signUpPwdRef}
                    className="w-full px-3 py-1.5 pr-10 text-sm lg:text-sm border border-slate-300 rounded-lg focus:outline-none focus:border focus:border-blue-500 transition-all"
                  ...
                  {showSignUpChecklist &&
                    (signUpPwdFocused ||
                      (form.signUp.password || "").length > 0) &&
                    !areAllPasswordRulesOk(form.signUp.password) && (
                      <div
                        className="absolute top-full left-0 mt-2 w-full bg-white border border-slate-200 rounded-lg shadow-lg p-2.5 sm:p-3 z-20"
                        role="dialog"
                        aria-live="polite"
                      >
                        ...
                        <ul className="space-y-1">
                          {getUnmetPasswordRules(form.signUp.password).map(
                            (r) => (
                              <li
                                key={r.id}
                                className="text-[10px] text-slate-600 flex items-start"
                              >
                ...
                <div className="flex items-center justify-center gap-2">
                  <input
                    ...
                    onChange={handleSignUpChange}
                    required
                    className="mt-0.5 sm:mt-1 w-3 h-3 text-blue-600 border-slate-300 rounded focus:ring-2 focus:ring-blue-500"
                  />
                  <label
        ...
        <div
          className={`hidden md:block absolute top-0 h-full w-1/2 transition-all duration-700 ease-in-out z-20 ${
            active ? "left-0" : "left-1/2"
          }`}
        >
          <div className="relative w-full h-full bg-gradient-to-br from-blue-600 to-blue-800 overflow-hidden rounded-5xl">
            <div
              className={`absolute inset-0 flex flex-col items-center justify-center text-white p-8 lg:p-12 transition-all duration-700 ${
                active
                  ? "translate-x-0 opacity-100"
                  : "translate-x-full opacity-0"
              }`}
            >
              <h3 className="text-xl lg:text-xl font-bold mb-3 lg:mb-4 text-center">
                Hello Again!
              </h3>
        ...
        <div className="md:hidden absolute bottom-6 sm:bottom-8 left-0 right-0 flex justify-center gap-3 sm:gap-4 px-4 z-30">
          <button
            type="button"
            onClick={() => setActive(false)}
            className={`px-5 sm:px-6 py-2 text-sm sm:text-base rounded-lg font-semibold transition-all ${
              !active
